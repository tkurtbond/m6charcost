...
(defun die (status msg)
  (format *error-output* "m6: ~A~%" msg)
  (uiop:quit status t))

(defun main ()
  (let* ((filename (trivia:match (ext:command-args)
                     ((list progname)
                      (die 1 (format nil "Too few arguments: ~S" (ext:command-args))))
                     ((list progname filename)
                      filename)
                     ((list* progname filename others)
                      (die 1 (format nil "Too many arguments: ~S" (ext:command-args))))))
         (characters-string (alexandria:read-file-into-string filename))
